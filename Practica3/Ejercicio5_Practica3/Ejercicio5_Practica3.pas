...
begin
	...
	while(d.codigo<> valorAlto) and(ok)do begin
		if(d.codigo= flor.codigo)and(d.nombre = flor.nombre) then begin // si encontramos el codigo
			flor.codigo:= FilePos(a) * -1;// le asignamos al codigo actual la posicion del elemento en el registro, pero en negativo
			flor.nombre:= '***'; // 
			seek(a,FilePos(a)-1);// escribo en la posicion a borrar la posicion del siguiente espacio libre.
			write(a,r0);
			seek(a,0);
			write(a,flor);// escribo en el principio del archivo la posicion del nuevo espacio libre
			ok:=false;
		end;
		if(ok)then 
...
procedure EliminarFlores (var a: tArchFlores);
var
	d:reg_flor;
begin
	writeln('ingrese codigo y nombre de la flor a eliminar');
	readln(d.codigo);
	readln(d.nombre);
	EliminarFlor(a,d);
end;
